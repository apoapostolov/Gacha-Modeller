...
JP 5.5 (29 Jul 2026) replaced Recruitment Points with Recruitment Charge. This lab encodes both as studied like-models. Rates are the published pickup table. The charge rules follow the JP wiki and Yostar’s 28 Jul writeup, not a scraped live banner.
...
P(pickup | 3-star) on a normal roll is 0.7 / 3 ≈ 23.3%. That is **not** the 50% at charge 100.
...
Planning case the community lost: natural A at 60, keep pulling, spark B at 200. Both students, 200 pulls.

Worst case for both was never 200. Spark spends the bar. A second student still needs another 200 or a natural. The nerf is the *banked leftover*, not the ceiling.
...
Dual same-type pickups share the bar, so getting A zeros the bar you wanted for B. That is why “both students” jumped from a 200-bank plan to two independent charge cycles.

## What the lab compares

| Preset | First pickup | Two sequential pickups |
| --- | --- | --- |
| `blue-archive-spark` | Spark at 200 if still missing | Leftover points stay; spark the second |
| `blue-archive-charge` | 100 coin, 200 hard, reset on hit | Second banner starts at charge 0 |

The UI runs both two-pickup arms when either preset is selected.

Unconditional mean pulls to *two* pickups can favor Charge. The 100-charge 50% 3-star is a real gift on every cycle, twice. The upheaval is leftover banking and the tail: if A lands by 80, Spark still caps both at 200, Charge can walk a second 200 (p90 and max go up). The lab reports that early-A slice separately. There is also no capturing, so a lost 100 does not arm the next 3-star. You also lose the spark *choice* of which concurrent pickup to take.
...
This is a like-model. If JP patched the 50% or the reset after these sources, the preset is stale on purpose until we cite a new shape.
